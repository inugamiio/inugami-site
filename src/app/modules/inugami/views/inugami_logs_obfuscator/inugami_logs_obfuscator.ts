...
@Component({
    templateUrl: './inugami_logs_obfuscator.html'
})
export class InugamiLogsObfuscatorView  implements  OnInit {
    ...
    **************************************************************************/
    ...
    ngOnInit(): void {
        this.googleAnalytics.pageView("/inugami/inugami_logs_obfuscator");
        this.httpService.getSourceCode("data/inugami.xml").then(data=> this.initData(data));
        this.httpService.getSourceCode("data/inugami_logs_obfuscator/source_code.xml").then(data=> this.initData(data));
