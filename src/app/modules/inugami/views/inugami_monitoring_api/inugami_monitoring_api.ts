...
@Component({
    templateUrl: './inugami_monitoring_api.html'
})
export class InugamiMonitoringApiView  implements  OnInit {
    ...
    **************************************************************************/
    ...
    ngOnInit(): void {
        this.googleAnalytics.pageView("/inugami/inugami_monitoring_api");
        this.httpService.getSourceCode("data/inugami.xml").then(data=> this.initData(data));
        this.httpService.getSourceCode("data/inugami_monitoring_api/source_code.xml").then(data=> this.initData(data));
