...
@Component({
    templateUrl: './inugami_monitoring_core.html'
})
export class InugamiMonitoringCoreView  implements  OnInit {
    ...
    **************************************************************************/
    ...
    ngOnInit(): void {
        this.googleAnalytics.pageView("/inugami/inugami_monitoring_core");
        this.httpService.getSourceCode("data/inugami.xml").then(data=> this.initData(data));
        this.httpService.getSourceCode("data/inugami_monitoring_core/source_code.xml").then(data=> this.initData(data));
